/*
 * Programa desarrollado por estudiante
 * El codigo puede ser mejorado en muchos aspectos
 * Sientase libre en repurar en caso de requerirlo
 */
...
public class Intercalacion {
    ...
    public void start() throws FileNotFoundException, IOException{
        ...
        System.out.println("\n\t ** ETAPA DE INTERCALACION INICIA **\n\n");
        
        /*
         * Se implementa la nomenclatura de libro de Joyanes 
         *                    "Estructura de datos en java", pag 219
         * por si se decea reforzar, la implementacion de este metodo
         */
        
        /* Vector que contiene el codigo de cada archivo*/
        int[] c = new int[m];
        
        /* Vector auxiliar que determina la posicion en el arreglo de archivos cvs*/
        this.deUltima = new int[m];
        
        /* Vector que contiene los tramos en cada archivo*/
        a = completar(distInicial.getNumTramos());
        
        /* Vector que contiene el total de ejecuciones a considerar en cada archivo*/
        int[] aC = iniciarNumEjecuciones(a.length);
        
        /* Vector que contiene los tramos ficticios de cada archivo*/
        int[] d = completar(distInicial.getNumTramosFicticios());
        
        /* no me acuerdo para que sirve*/
        int[] cd = new int[m];
        
        /* cantidad de registros por tramo*/
        int[] actv = new int[m];
        encerar(actv);
        ...
        for(int i = 0; i < m;i++){
            ...
            if(i!=m-1){
                ...
                System.out.println("   archivoT"+String.valueOf(i+1)+".csv");
                
                archivosLectura[i] = 
                    new CsvReader(new FileReader("archivoT"+(i+1)+".csv"));   
            }
        }
        
        
        /* Número de tramos a mezclar en cada pasada pasada */
        int z;
        
        // El nivel calculado en la distribución inicial, representa el maximo
        // de iteraciones en el polifase
        for(int j = 0; j < distInicial.getNivel();j++){
            ...
            ArchivoEntrada = new CsvWriter("archivoT"+c[m-1]+".csv");
            ...
            archivosLectura[this.deUltima[c[m-2]-1]] = new CsvReader(new FileReader("archivoT"+c[m-1]+".csv"));
...
// SE CREA UN Switch entre el archivo salida y de entrada para los nuevos tramos
    ...
    private void verVector(String mensaje,int [] vector){
