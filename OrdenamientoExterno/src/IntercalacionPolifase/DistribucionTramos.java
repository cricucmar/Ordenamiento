...
public class DistribucionTramos {
    ...
    public void start() throws FileNotFoundException, IOException, ParseException {
        ...
        for(int j = 0;j < this.numFileOut;j++){
            System.out.println(" >> CREANDO EL ARHIVO: archivoT"+String.valueOf(j+1)+".csv");
            ArchivoDeEscritura[j] = new CsvWriter("archivoT"+String.valueOf(j+1)+".csv");
        }
        ...
        System.out.println("\n\t**RESUMEN ETAPA DISTRIBUCION**\n");
        verVector(" DISTRIBUIDOS FINAL : ",this.numTramos);
        verVector(" TRAMOS FICTICIOS   : ",this.numTramosFicticios);
        System.out.println(" REGISTOS LEIDOS     : "+this.totalTramos + 
                           " NIVEL: " + this.nivel );
        
        // Se cierra el Archivo Base
        archivoBase.close();
                        
        // Se Cierra los Archivos de salida
        for(int j = 0;j < this.numFileOut;j++){
